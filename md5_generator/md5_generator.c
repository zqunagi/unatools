...
typedef enum { ARG_HELP, ARG_VERSION, ARG_PATH, ARG_UNKNOWN } ArgumentType;
typedef enum { PATH_REGULARFILE, PATH_DIRECTORY, PATH_OTHER } PathType;
typedef struct {
  const char *option;
  ArgumentType type;
} OptionMap;

OptionMap options[] = {{"-h", ARG_HELP},    {"--help", ARG_HELP},
                       {"-v", ARG_VERSION}, {"--version", ARG_VERSION},
                       {"-p", ARG_PATH},    {"-path", ARG_PATH},
                       {NULL, ARG_UNKNOWN}};
...
void print_help() {
  ...
  printf("Description:\n");
  printf(
      "  This program generates md5 digest for a signal file or all files in a "
      "folder.\n\n");
  printf("Options:\n");
  printf("  -h, --help                Display this help message\n");
  printf("  -v, --version             Display version information\n");
  printf(
      "  -p, --path <path>     	Specify the path of the file or "
      "folder\n");
}
